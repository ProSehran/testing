...
def check_ip_virustotal(api_key, ip_address):
    ...
    try:
        ...
        if response.status_code == 200:
            if mal >= 1:
                result = ("Malicious", whois_date, last_modification_date, country_fullname, as_owner, link, last_analysis_stats)
            elif sus >= 1:
                result = ("Suspicious", whois_date, last_modification_date, country_fullname, as_owner, link, last_analysis_stats)
            else:
                result = ("Clean", whois_date, last_modification_date, country_fullname, as_owner, link, last_analysis_stats)
            return 200, result
        elif response.status_code == 400:
            print(f"\nIP {ip_address} returned a 400 status code.\n")
            return 400, None
        else:
    ...
    for index in range(start_index - 1, len(df)):
        ...
        if is_valid_ip(ip_address):
            if pd.isnull(status):
                ...
                # Saving the dataframe after each iteration
                df.to_csv('dif_test.csv', index=False)  
            else:
