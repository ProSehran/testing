...
api_token = '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'
repo_owner = 'tachyonsecurity'
repo_name = 'Security_IOC_private'
branch_name = 'main'
...
def callAPI(api_key, df, start_index, type_of_scan):
    for index in range(start_index - 1, len(df)):
        ...
        if is_valid_ip(ip_address):
            if type_of_scan == "diff":
                if pd.isnull(status or link):
                ...
                print("Invalid type of scan. Please enter 'full' or 'diff'.")
...
def main():
    ...
    start_index = 1
    type_of_scan = input("\n Enter the type of scan you want to perform (full/diff): ")

    callAPI(api_key, df, start_index, type_of_scan)
